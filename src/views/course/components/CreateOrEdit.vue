    ...
    <el-card>
        <div slot="header">
          <el-steps :active="activeStep" simple>
            <el-step v-for="(item, i) in steps" :key="item.id" :title="item.title" :icon="item.icon" @click.native="activeStep = i"></el-step>
          </el-steps>
        </div>
      <!--步骤对应的表单结构-->
      <el-form label-width="80px">
        ...
        <div v-show="activeStep === 0">
          ...
          <el-form-item label="课程概述">
            <el-input placeholder="概述1" v-model="course.previewFirstField" style="width: 49%;min-width: 300px; margin-right: 15px">
              <template slot="append">{{ course.previewFirstField.length }} / 20</template>
            </el-input>
            <el-input placeholder="概述2" v-model="course.previewSecondField" style="width: 49%;min-width: 300px">
              <template slot="append">{{ course.previewFirstField.length }} / 20</template>
            </el-input>
          </el-form-item>
          ...
          <el-form-item label="课程排序">
            <el-input-number label="描述文字" controls-position="right" v-model="course.sortNum"></el-input-number>
          </el-form-item>
        </div>
        <!--课程封面-->
         ...
         <el-form-item label="售卖价格">
          ...
          <el-form-item label="活动标签">
            <el-input v-model="course.discountsTag">
              <template slot="append">{{ course.discountsTag.length }} / 100</template>
            </el-input>
          </el-form-item>
        </div>
        <!--秒杀信息-->
        ...
        <div v-show="activeStep === 4">
          ...
          <el-form-item label="是否上架">
            <el-switch active-color="#13ce66" inactive-color="#ff4949" :active-value="1" :inactive-value="0" v-model="course.status"></el-switch>
          </el-form-item>
          <el-form-item>
...
export default {
  ...
  created () {
    // 检测当前是否为编辑功能，如果是，根据courseId 请求课程数据
    if (this.isEdit) {
      this.loadCourse()
    }
  },
  data () {
    return {
      ...
      steps: [
        ...
        { id: 3, title: '销售信息', icon: 'el-icon-wallet' },
        { id: 4, title: '秒杀信息', icon: 'el-icon-wallet' },
        { id: 5, title: '课程详情', icon: 'el-icon-data-line' }
      ],
      // 本地预览图片地址
      imageUrl: '',
      // 添加课程的相关信息
  ...
  methods: {
    ...
    async loadCourse () {
      ...
      if (data.code === '000000') {
        // 判断当前课程是否为开启秒杀的状态，如果没有秒杀，需要初始化数据
